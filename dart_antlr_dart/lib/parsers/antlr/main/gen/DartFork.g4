...
@parser::members {
  ...
  // Returns whether there's no skipped token between the previous
  // and the current visible token. A visible token is a token that
  // was not skipped.
  // The current skipping approach removes such tokens from the
  // token stream, which makes this approach possible.
  bool isNoSkip() {
    return tokenStream.LT(-1)!.stopIndex + 1 == tokenStream.LT(1)!.startIndex;
  }

  // TODO have trailing actions for verifying whether the operators have spaces in them.
}
